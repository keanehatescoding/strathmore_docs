By default when we enter a Cisco switch we enter __user EXEC__ mode which has a prompt as shown below
```shell
R1>
```
We can enter a more privileged execution mode called __privileged execution mode__ by typing enable as shown below
```shell
R1>enable
R1#
```
...
To protect privileged  mode with an unencrypted password type:
```shell
enable
configure terminal
...
Virtual terminals lines/Virtual teletype(VTY) allow remote access using telnet or SSH. Many Cisco switches support up to 16 VTY lines numbered 0 to 15.
...
S1#
```
To ensure all entered password are of a min-length type:
```shell
security passwords min-length
```
To block a client for 60s after 3 failed attempts within 60 mins type:
```shell
login block-for 60 attempts 3 within 60
```
To make a user to be automatically logged out for 5 mins after 60 seconds of inactivity type
```shell
exec-timeout 5 60
```
