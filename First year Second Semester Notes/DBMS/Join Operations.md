...
This constitutes a collection of several join operations denoted as $\theta$ i.e greater than, greater than or equal to, less than, less than or equal to, not equal to, equal to etc;
...
$$\text{R}\bowtie_{\theta}S=\sigma_{\theta}(R\times S)$$

### Oracle PL/SQL Equivalent Expression

```SQL
SELECT * FROM tblName1,tblName2 WHERE tblName1 
```
...
The left Outer Join returns a relation with all tuples from the left relation and matching tuples from the right relation.
If no match is found, NULLS are returned for the columns on the right relation.
The right Outer Join returns a relation with all tuples from the right relation and matching  tuples from the left relation.
If no matches are found for the left relations then NULLS are returned for the columns on the left relation.

### 2. Full Outer Join

It returns all rows where there is a match in either table. Rows with no matches will have NULLS in the columns of the other table

### 3. The Inner Join

Results a relation with tuples that satisfy the matching criteria.
It discards tuples of either relations that do not match the any tuple of the other relation.
...
It returns a relation that contains only the tuples from R that participate in the join with S but does not return columns from the S.
